...
当前时序存在 mermaid 高度测不准的问题：

```
现状：
  splitMarkdownBlocks → useBlockHeights 实测（mermaid 还是占位符！）
  → 分页 → parseMarkdownAsync（此时 mermaid 才真正渲染）
```

修正为：

```
parseMarkdownAsync(md) → 完整 HTML（mermaid 已是真实 SVG，带自然尺寸）
→ useBlockHeights 实测 → 拿到 mermaid 真实高度
→ 分页：图矮则正常与其他块共页；图高则独占一页/缩放
```

即：**测量容器的渲染入口必须从 `parseMarkdown` 改为 `parseMarkdownAsync`**，让占位符在实测前被替换成真实 SVG。`estimatedHeight` 给保守默认值 `280px`，实测值覆盖它。

### 5.2 A4 模式（双层保障）

1. **估算层**（`documentModel.ts`）：`estimateBlockHeight` 对 `kind === 'mermaid'` 返回 `280`，`avoidBreak: true`。初次分页把 mermaid 当原子块。
2. **实测层**（`useBlockHeights`）：测量容器改走 `parseMarkdownAsync`，mermaid 已是真实 SVG。
...
| `src/engine/utils/markdownParser.ts` | mermaid 代码块分支：检测 `lang === 'mermaid'`，从 mermaidMap 取 SVG 或降级；新增 `collectMermaidDiagrams` + `preRenderMermaid`；`parseMarkdownAsync` 新增 `containerWidth` 参数 |
...
| `src/modes/document/paged/pagedContent.ts` | `parseMarkdown` → `parseMarkdownAsync`（传 A4 containerWidth）；保留原有 normalize/caption 逻辑 |
| `src/modes/document/paged/usePagedPreview.ts` | 渲染完成后遍历 mermaid 块做缩放兜底 |
...
| `src/modes/card/CardMode.tsx` | 隐藏测量容器渲染入口改 `parseMarkdownAsync`；Card containerWidth |
