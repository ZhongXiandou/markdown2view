# 图片导出

<cite>
**本文引用的文件**
- [exportImage.ts](file://src/lib/exportImage.ts)
- [clipboard.ts](file://src/lib/clipboard.ts)
- [export/index.ts](file://src/lib/export/index.ts)
- [export/longImage.ts](file://src/lib/export/longImage.ts)
- [export/zipDownload.ts](file://src/lib/export/zipDownload.ts)
- [exportPdf.ts](file://src/lib/exportPdf.ts)
- [store.ts](file://src/lib/store.ts)
- [SettingsModal.tsx](file://src/components/editor/SettingsModal.tsx)
- [EditorToolbar.tsx](file://src/components/editor/EditorToolbar.tsx)
- [imageStorage.ts](file://src/lib/editor/imageStorage.ts)
- [extractHtml.ts](file://src/lib/extractHtml.ts)
</cite>

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能考量](#性能考量)
8. [故障排除指南](#故障排除指南)
9. [结论](#结论)
10. [附录](#附录)

## 简介
本文件系统性阐述 markdown2view 的图片导出能力，重点围绕以下方面：
- 使用 modern-screenshot 库进行 DOM 元素捕获、Canvas 渲染与图像生成的技术细节
- 图片导出的质量控制机制：分辨率、压缩与格式选择（PNG/JPEG/WebP）
- 剪贴板集成：将图片复制到剪贴板的技术实现与浏览器兼容性处理
- 导出配置项详解：画布尺寸、背景色、透明度与质量参数
- 大图导出的内存优化策略与性能权衡
- 导出失败的故障排除与错误处理机制

## 项目结构
与图片导出相关的关键模块分布如下：
- 导出核心：exportImage.ts 提供 iframe/元素截图、下载与长图导出入口
- 剪贴板：clipboard.ts 提供富文本/HTML/文本复制能力
- 导出聚合：export/index.ts 汇总导出 API
- 扩展导出：export/longImage.ts、export/zipDownload.ts 提供长图与打包下载
- PDF 导出：exportPdf.ts 使用 modern-screenshot 的 JPEG 能力生成 PDF
- 存储与配置：store.ts 与 SettingsModal.tsx 管理图床配置
- 图片存储与压缩：imageStorage.ts 提供 Canvas 压缩与本地 URL 解析
- HTML 提取：extractHtml.ts 注入跨域支持以提升截图稳定性

```mermaid
graph TB
subgraph "导出核心"
EI["exportImage.ts<br/>iframeToBlob / elementToBlob / captureElementInIframeToBlob"]
EX["export/index.ts<br/>导出 API 聚合"]
LI["export/longImage.ts<br/>长图导出"]
ZD["export/zipDownload.ts<br/>ZIP 打包下载"]
end
subgraph "剪贴板"
CB["clipboard.ts<br/>copyRichText / copyHtmlSource / copyText"]
end
subgraph "PDF 导出"
EP["exportPdf.ts<br/>domToJpeg 生成 PDF"]
end
subgraph "配置与存储"
ST["store.ts<br/>imageHostConfig"]
SM["SettingsModal.tsx<br/>图床配置界面"]
IS["imageStorage.ts<br/>Canvas 压缩/本地 URL"]
EH["extractHtml.ts<br/>@font-face 跨域支持"]
end
EX --> EI
LI --> EI
ZD --> EI
EP --> EI
CB --> IS
ST --> SM
EH --> EI
```

图表来源
- [exportImage.ts:152-385](file://src/lib/exportImage.ts#L152-L385)
- [export/index.ts:1-3](file://src/lib/export/index.ts#L1-L3)
- [export/longImage.ts:16-28](file://src/lib/export/longImage.ts#L16-L28)
- [export/zipDownload.ts:11-34](file://src/lib/export/zipDownload.ts#L11-L34)
- [exportPdf.ts:137-182](file://src/lib/exportPdf.ts#L137-L182)
- [clipboard.ts:4-131](file://src/lib/clipboard.ts#L4-L131)
- [store.ts:41-52](file://src/lib/store.ts#L41-L52)
- [SettingsModal.tsx:11-52](file://src/components/editor/SettingsModal.tsx#L11-L52)
- [imageStorage.ts:76-120](file://src/lib/editor/imageStorage.ts#L76-L120)
- [extractHtml.ts:54-54](file://src/lib/extractHtml.ts#L54-L54)

章节来源
- [exportImage.ts:152-385](file://src/lib/exportImage.ts#L152-L385)
- [export/index.ts:1-3](file://src/lib/export/index.ts#L1-L3)
- [export/longImage.ts:16-28](file://src/lib/export/longImage.ts#L16-L28)
- [export/zipDownload.ts:11-34](file://src/lib/export/zipDownload.ts#L11-L34)
- [exportPdf.ts:137-182](file://src/lib/exportPdf.ts#L137-L182)
- [clipboard.ts:4-131](file://src/lib/clipboard.ts#L4-L131)
- [store.ts:41-52](file://src/lib/store.ts#L41-L52)
- [SettingsModal.tsx:11-52](file://src/components/editor/SettingsModal.tsx#L11-L52)
- [imageStorage.ts:76-120](file://src/lib/editor/imageStorage.ts#L76-L120)
- [extractHtml.ts:54-54](file://src/lib/extractHtml.ts#L54-L54)

## 核心组件
- 导出选项类型 ImageOpts：定义缩放比例、输出类型（PNG/JPEG/WebP）、背景色与最大高度
- DOM 就绪等待：waitForDocumentReady 与 waitForStability 确保字体、样式、图片加载完成并达到稳定
- 背景色解析：resolveBackground 优先使用显式背景色，否则从 body/html 计算，兜底白色
- iframe 截图：iframeToBlob 将 iframe 内容撑高至全高，计算布局宽高后调用 domToBlob
- 元素截图：elementToBlob 获取元素尺寸后截图
- 元素内截图：captureElementInIframeToBlob 通过临时样式对齐 iframe/html/body/目标元素，保证截图与元素尺寸一致
- 下载：downloadBlob 统一触发下载并释放对象 URL
- 长图导出：exportLongImage 用于“长图文”模式导出整篇文章为单张高图
- ZIP 打包：downloadAsZip 将多个 Blob 打包为 ZIP 并下载，失败时回退顺序下载
- PDF 导出：exportElementsToPdf 使用 domToJpeg 生成多页 PDF

章节来源
- [exportImage.ts:16-21](file://src/lib/exportImage.ts#L16-L21)
- [exportImage.ts:61-117](file://src/lib/exportImage.ts#L61-L117)
- [exportImage.ts:119-138](file://src/lib/exportImage.ts#L119-L138)
- [exportImage.ts:152-197](file://src/lib/exportImage.ts#L152-L197)
- [exportImage.ts:199-217](file://src/lib/exportImage.ts#L199-L217)
- [exportImage.ts:250-385](file://src/lib/exportImage.ts#L250-L385)
- [exportImage.ts:219-228](file://src/lib/exportImage.ts#L219-L228)
- [export/longImage.ts:16-28](file://src/lib/export/longImage.ts#L16-L28)
- [export/zipDownload.ts:11-34](file://src/lib/export/zipDownload.ts#L11-L34)
- [exportPdf.ts:137-182](file://src/lib/exportPdf.ts#L137-L182)

## 架构总览
图片导出的整体流程：等待 DOM 稳定 → 计算布局尺寸 → 设置背景色与缩放 → 调用 modern-screenshot 的 domToBlob/domToJpeg → 生成 Blob → 下载或进一步处理（PDF/ZIP/长图）。

```mermaid
sequenceDiagram
participant U as "用户"
participant E as "导出入口<br/>exportImage.ts"
participant M as "modern-screenshot<br/>domToBlob/domToJpeg"
participant D as "下载器<br/>downloadBlob"
U->>E : "请求导出iframe/元素/元素内"
E->>E : "waitForDocumentReady/waitForStability"
E->>E : "resolveBackground/计算布局宽高"
E->>M : "domToBlob(dom.documentElement) 或 domToJpeg(元素)"
M-->>E : "返回 Blob"
E->>D : "downloadBlob(blob, filename)"
D-->>U : "触发浏览器下载"
```

图表来源
- [exportImage.ts:61-117](file://src/lib/exportImage.ts#L61-L117)
- [exportImage.ts:152-197](file://src/lib/exportImage.ts#L152-L197)
- [exportImage.ts:199-217](file://src/lib/exportImage.ts#L199-L217)
- [exportImage.ts:250-385](file://src/lib/exportImage.ts#L250-L385)
- [exportImage.ts:219-228](file://src/lib/exportImage.ts#L219-L228)

## 详细组件分析

### 组件A：DOM 就绪与稳定性检测
- waitForDocumentReady：等待文档 readyState 完成、样式表加载完成、字体可用、图片 decode 成功，并再次等待 DOM 变动稳定
- waitForStability：基于 MutationObserver 与帧渲染，持续监测 DOM 变化，超过阈值判定稳定
- 作用：避免截图时字体未加载、图片未解码、布局未稳定导致的失真或截断

```mermaid
flowchart TD
S["开始"] --> R1["检查文档就绪"]
R1 --> L1["等待样式表/字体/图片加载"]
L1 --> ST["启动 MutationObserver 稳定计时器"]
ST --> CHG{"DOM 是否持续变化？"}
CHG --> |是| ST
CHG --> |否| OK["达到稳定状态"]
OK --> E["结束"]
```

图表来源
- [exportImage.ts:61-117](file://src/lib/exportImage.ts#L61-L117)
- [exportImage.ts:27-59](file://src/lib/exportImage.ts#L27-L59)

章节来源
- [exportImage.ts:61-117](file://src/lib/exportImage.ts#L61-L117)
- [exportImage.ts:27-59](file://src/lib/exportImage.ts#L27-L59)

### 组件B：背景色解析与透明度处理
- resolveBackground：按显式覆盖优先，其次内联样式，再计算 getComputedStyle，最后兜底白色
- 透明度策略：若显式透明或 rgba(0,0,0,0)，则不覆盖背景，交由浏览器默认背景色
- 影响：确保截图背景符合预期，避免 PNG/WebP 透明导致的视觉差异

```mermaid
flowchart TD
A["输入：Document/Window/可选覆盖色"] --> O{"是否提供覆盖色？"}
O --> |是| RET1["返回覆盖色"]
O --> |否| B1["尝试 body 内联背景"]
B1 --> B2{"为空或透明？"}
B2 --> |是| C1["计算 body 计算样式"]
B2 --> |否| RET2["返回 body 计算背景"]
C1 --> C2{"为空或透明？"}
C2 --> |是| D1["计算 html 计算样式"]
C2 --> |否| RET3["返回 html 计算背景"]
D1 --> D2{"为空或透明？"}
D2 --> |是| DEF["返回默认白色"]
D2 --> |否| RET4["返回 html 计算背景"]
```

图表来源
- [exportImage.ts:119-138](file://src/lib/exportImage.ts#L119-L138)

章节来源
- [exportImage.ts:119-138](file://src/lib/exportImage.ts#L119-L138)

### 组件C：iframe 截图（全页）
- 关键步骤：将 iframe 高度设为内容全高，隐藏滚动，计算布局宽高，调用 domToBlob
- 参数：scale、type、backgroundColor、width、height、fetch.cache
- 最大高度保护：根据 scale 限制 maxHeight，避免超大图导致内存溢出

```mermaid
sequenceDiagram
participant IF as "iframe"
participant DOC as "contentDocument"
participant W as "Window"
participant E as "iframeToBlob"
participant M as "domToBlob"
E->>W : "waitForDocumentReady"
E->>DOC : "设置 overflow/高度为全高"
E->>E : "计算 layoutWidth/layoutHeight"
E->>M : "domToBlob(documentElement, {scale,type,bg,width,height})"
M-->>E : "返回 Blob"
E->>DOC : "恢复原样式"
```

图表来源
- [exportImage.ts:152-197](file://src/lib/exportImage.ts#L152-L197)

章节来源
- [exportImage.ts:152-197](file://src/lib/exportImage.ts#L152-L197)

### 组件D：元素截图（局部）
- 关键步骤：等待元素稳定，获取元素自然尺寸，调用 domToBlob
- 注意：元素无尺寸时抛错，需确保元素已渲染

```mermaid
flowchart TD
S["开始"] --> W["等待元素稳定"]
W --> G["获取元素尺寸"]
G --> V{"宽/高有效？"}
V --> |否| ERR["抛出错误：导出节点暂无尺寸"]
V --> |是| C["调用 domToBlob(元素)"]
C --> R["返回 Blob"]
```

图表来源
- [exportImage.ts:199-217](file://src/lib/exportImage.ts#L199-L217)

章节来源
- [exportImage.ts:199-217](file://src/lib/exportImage.ts#L199-L217)

### 组件E：元素内截图（精准裁切）
- 核心原理：临时重置缩放与自适应，将 iframe、html、body、目标元素全部对齐到元素自然尺寸，消除 margin/padding/flex 居中等干扰，再对 documentElement 截图
- 优势：截图与元素尺寸完全一致，保留全局样式与 CSS 变量
- 回滚：finally 中恢复所有临时样式

```mermaid
flowchart TD
S["开始"] --> RD["等待文档就绪"]
RD --> T1["临时设置 iframe 宽高/最大宽高"]
T1 --> T2["重置 body 缩放/--auto-scale"]
T2 --> M["测量元素自然宽高"]
M --> A1["对 iframe/html/body/元素设置样式对齐"]
A1 --> ST["等待 DOM 稳定"]
ST --> CAP["domToBlob(documentElement)"]
CAP --> RES["返回 Blob"]
RES --> RST["finally 恢复样式与 iframe 尺寸"]
```

图表来源
- [exportImage.ts:250-385](file://src/lib/exportImage.ts#L250-L385)

章节来源
- [exportImage.ts:250-385](file://src/lib/exportImage.ts#L250-L385)

### 组件F：下载与批量导出
- downloadBlob：统一触发下载并释放对象 URL
- exportLongImage：长图导出，适合导出整篇长文为单张高图
- downloadAsZip：将多个 Blob 打包为 ZIP，失败时回退顺序下载

```mermaid
sequenceDiagram
participant U as "用户"
participant L as "exportLongImage"
participant E as "elementToBlob"
participant D as "downloadBlob"
U->>L : "导出长图"
L->>E : "elementToBlob(元素, {scale, backgroundColor})"
E-->>L : "返回 Blob"
L->>D : "downloadBlob(blob, filename)"
D-->>U : "下载完成"
```

图表来源
- [export/longImage.ts:16-28](file://src/lib/export/longImage.ts#L16-L28)
- [exportImage.ts:219-228](file://src/lib/exportImage.ts#L219-L228)

章节来源
- [export/longImage.ts:16-28](file://src/lib/export/longImage.ts#L16-L28)
- [export/zipDownload.ts:11-34](file://src/lib/export/zipDownload.ts#L11-L34)
- [exportImage.ts:219-228](file://src/lib/exportImage.ts#L219-L228)

### 组件G：剪贴板集成
- copyText：优先使用 Clipboard API，失败时降级为 textarea + execCommand
- copyRichText：克隆元素，将本地图片（blob:/img://）编译为 base64，构造 HTML+纯文本 ClipboardItem，优先 Clipboard API，失败回退 execCommand
- copyHtmlSource：将元素内图片编译为 base64 后复制 HTML 源码

```mermaid
sequenceDiagram
participant U as "用户"
participant C as "copyRichText"
participant CL as "compileElementImages"
participant N as "navigator.clipboard"
U->>C : "复制富文本"
C->>CL : "克隆并编译本地图片为 base64"
CL-->>C : "返回已编译元素"
C->>N : "write(HTML+纯文本)"
alt 失败
C->>C : "降级：选区 + execCommand"
end
C-->>U : "返回是否成功"
```

图表来源
- [clipboard.ts:64-100](file://src/lib/clipboard.ts#L64-L100)
- [clipboard.ts:32-61](file://src/lib/clipboard.ts#L32-L61)

章节来源
- [clipboard.ts:4-131](file://src/lib/clipboard.ts#L4-L131)

### 组件H：PDF 导出（JPEG）
- 使用 domToJpeg 生成高质量 JPEG 数据 URL，再用 jsPDF 添加页面并保存
- 适用场景：非 iframe 的元素导出 PDF（如 A4 文档模式）

```mermaid
sequenceDiagram
participant U as "用户"
participant EP as "exportElementsToPdf"
participant MJ as "domToJpeg"
participant J as "jsPDF"
U->>EP : "导出 PDF"
EP->>MJ : "domToJpeg(元素, {scale=3, backgroundColor})"
MJ-->>EP : "返回 dataUrl"
EP->>J : "addImage(dataUrl) 并保存"
J-->>U : "PDF 文件"
```

图表来源
- [exportPdf.ts:137-182](file://src/lib/exportPdf.ts#L137-L182)

章节来源
- [exportPdf.ts:137-182](file://src/lib/exportPdf.ts#L137-L182)

## 依赖关系分析
- 导出核心依赖 modern-screenshot 的 domToBlob/domToJpeg 与 waitUntilLoad
- HTML 提取模块为 @font-face 注入跨域属性，提升字体嵌入成功率
- 图片存储模块提供 Canvas 压缩与本地 URL 解析，配合剪贴板将本地图片编译为 base64
- 导出聚合模块统一导出 API，便于上层组件调用

```mermaid
graph LR
EI["exportImage.ts"] --> MS["modern-screenshot<br/>domToBlob/domToJpeg/waitUntilLoad"]
EH["extractHtml.ts"] --> EI
IS["imageStorage.ts"] --> CB["clipboard.ts"]
EX["export/index.ts"] --> EI
LI["export/longImage.ts"] --> EI
ZD["export/zipDownload.ts"] --> EI
EP["exportPdf.ts"] --> MS
```

图表来源
- [exportImage.ts:7-7](file://src/lib/exportImage.ts#L7-L7)
- [exportPdf.ts:137-137](file://src/lib/exportPdf.ts#L137-L137)
- [extractHtml.ts:54-54](file://src/lib/extractHtml.ts#L54-L54)
- [imageStorage.ts:104-120](file://src/lib/editor/imageStorage.ts#L104-L120)
- [export/index.ts:1-1](file://src/lib/export/index.ts#L1-L1)
- [export/longImage.ts:1-1](file://src/lib/export/longImage.ts#L1-L1)
- [export/zipDownload.ts:1-1](file://src/lib/export/zipDownload.ts#L1-L1)

章节来源
- [exportImage.ts:7-7](file://src/lib/exportImage.ts#L7-L7)
- [exportPdf.ts:137-137](file://src/lib/exportPdf.ts#L137-L137)
- [extractHtml.ts:54-54](file://src/lib/extractHtml.ts#L54-L54)
- [imageStorage.ts:104-120](file://src/lib/editor/imageStorage.ts#L104-L120)
- [export/index.ts:1-1](file://src/lib/export/index.ts#L1-L1)
- [export/longImage.ts:1-1](file://src/lib/export/longImage.ts#L1-L1)
- [export/zipDownload.ts:1-1](file://src/lib/export/zipDownload.ts#L1-L1)

## 性能考量
- 缩放比例（scale）：默认 2，可按需调整；更高缩放带来更清晰图像但显著增加内存占用与生成时间
- 最大高度（maxHeight）：默认安全上限约为 16000/scale，避免超大图导致内存溢出
- 跨域字体与资源：extractHtml.ts 注入跨域属性，减少截图失败与重复网络请求
- 缓存策略：modern-screenshot 的 fetch.cache 设为 force-cache，降低重复资源加载
- 大图优化建议：
  - 优先使用元素内截图，避免一次性渲染整页
  - 适当降低 scale 或分段导出
  - 使用 WebP（若目标平台支持）以获得更好压缩比
  - 导出后及时释放对象 URL，避免内存泄漏

## 故障排除指南
- 截图失败（空 Blob）
  - 检查元素是否可见且有尺寸（元素截图）
  - 确认 iframe 已就绪、内容已渲染（iframe 截图）
  - 等待字体/图片/样式表加载完成（waitForDocumentReady）
  - 减小 scale 或设置 maxHeight 以避免内存不足
- 背景异常
  - 显式指定 backgroundColor 覆盖 resolveBackground 的默认行为
  - 若需要透明背景，确保输出类型为 PNG/WebP（JPEG 不支持透明）
- 剪贴板失败
  - 浏览器可能阻止非 HTTPS 或非用户手势触发的剪贴板访问
  - 降级路径：使用 textarea + execCommand
- 图床与本地图片
  - 本地图片为虚拟链接，复制 HTML 到某些平台可能导致图片失效
  - 建议配置图床（SM.MS、OSS、COS）或使用 Canvas 压缩后 base64 内联

章节来源
- [exportImage.ts:152-197](file://src/lib/exportImage.ts#L152-L197)
- [exportImage.ts:199-217](file://src/lib/exportImage.ts#L199-L217)
- [exportImage.ts:119-138](file://src/lib/exportImage.ts#L119-L138)
- [clipboard.ts:4-27](file://src/lib/clipboard.ts#L4-L27)
- [SettingsModal.tsx:100-109](file://src/components/editor/SettingsModal.tsx#L100-L109)

## 结论
本项目通过 modern-screenshot 实现了稳定、高质量的图片导出能力，结合完善的 DOM 就绪与稳定性检测、背景色解析、元素内精准裁切与统一下载机制，覆盖了从局部到整页、从静态到长图、从本地到云端的多种导出需求。配合剪贴板与图床配置，用户可在不同平台与场景下高效完成内容分享与归档。

## 附录

### 图片导出配置选项说明
- scale：缩放比例，默认 2，越大越清晰但内存与耗时越高
- type：输出类型，默认 PNG，可选 JPEG/WebP
- backgroundColor：背景色覆盖，默认从页面解析，透明将保持透明
- maxHeight：最大高度（像素），受 scale 影响的安全上限

章节来源
- [exportImage.ts:16-21](file://src/lib/exportImage.ts#L16-L21)
- [exportImage.ts:176-178](file://src/lib/exportImage.ts#L176-L178)