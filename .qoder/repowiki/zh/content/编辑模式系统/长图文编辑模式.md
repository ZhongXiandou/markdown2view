...
## 简介
长图文编辑模式旨在为公众号、知识长图与图文平台提供沉浸式长文创作体验。其通过“编辑器 + 实时预览”的双栏布局，结合滚动联动、防抖同步、富文本复制与长图导出能力，显著提升长文阅读与发布效率。该模式在渲染层面复用统一的 Markdown 解析内核，预览区具备标题/摘要元信息展示、字体选择、本地图片风险提示与多种复制/导出能力。
...
- 元信息展示与复制
  - 展示标题与摘要，并提供一键复制按钮。
- 渲染与字体
  - 从渲染结果提取 HTML，应用统一字体配置（宋体/仿宋/黑体/霞鹜文楷）。
...
### 渲染与元信息提取
- 渲染入口
  - renderMarkdown 接收 Markdown 文本与主题色，返回 { html, meta }。
- 元信息
  - 从 Markdown 中抽取标题、摘要等元信息，用于预览区顶部展示与复制。

```mermaid
flowchart TD
In(["输入 Markdown"]) --> Parse["parseMarkdown<br/>生成 HTML"]
In --> Extract["extractContentMeta<br/>提取 title/summary/contentMarkdown"]
Extract --> Merge["组合 {html, meta}"]
Merge --> Out(["输出渲染结果"])
```
...
**章节来源**
- [markdown.ts:4-15](file://src/lib/render/markdown.ts#L4-L15)
...
- 字体加载
  - 字体族通过 CSS 字体栈指定，尽量利用系统字体与本地字体资源，减少额外字体加载开销。
...
## 结论
长图文编辑模式通过“编辑器 + 预览”的双栏设计、滚动联动与文档同步，实现了高效稳定的长文创作体验。其渲染内核统一、工具链完备（复制富文本、HTML 源码、AI 指令、长图导出），并针对本地图片风险与字体配置提供了实用的优化措施。相比文档/卡片/HTML 模式，长图文模式更专注于社交平台与长图场景的阅读与发布需求。
...
- 样式调整
  - 通过工具栏选择字体（宋体/仿宋/黑体/霞鹜文楷），匹配平台风格。
...
- 与其他模式的区别
  ...
  - 长图文模式：专注长文阅读体验与社交平台发布，提供复制/导出/字体等专用能力。
