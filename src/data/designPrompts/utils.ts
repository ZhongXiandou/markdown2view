...
6. **16:9 幻灯片（Slide）严格高度限额（防垂直溢出）**：在 16:9 画布下（基准宽 960px，高 540px），垂直可用高度极其有限。严禁页面内容高度超出 380px（即容器总高度的 70%）。必须限制外层边距安全上限：\`.canvas-card\` 内边距最多为 \`padding: 32px 24px 24px\`，顶部页眉 \`.chrome-min\` 的 \`margin-bottom\` 严格限制在 \`16px\` 以内，底部页脚 \`.foot-min\` 的 \`margin-top\` 严格限制在 \`14px\` 以内。
7. **代码块极简规范**：在多卡片或幻灯片中，代码块必须精炼，代码段行数严格控制在 **12行** 以内，若超出必须压缩、截取或通过括号合并等方式压减行数。必须使用紧凑样式：\`font-size: 11px\`，\`line-height: 1.45\`，\`padding: 12px 16px\`。
8. **中文标题大号安全阈值**：为了防止在页面被垂直压缩时大字号标题折行导致页面整体高度溢出，中文大标题禁止使用绝对死大字号（如 \`48px\`/\`60px\` 或单一的大 \`vw\`），必须设置双向视口比例约束限制（如 \`font-size: min(5.2vw, 9.2vh)\` 或 \`font-size: clamp(24px, 4.5vw, 42px)\`）。
