...
9. **遵循原文原则**：尽量遵循用户提供的原文内容，如无必要，不得随意更改用户内容。仅做必要的格式调整和排版优化，不要修改原文的核心事实、数据、名称和表述。
10. **符号避让规则（防止标签误解析）**：
   - 正文避免使用**英文双引号**（即半角引号 \`"\`，如 "hello"），改用**中文双引号**（即全角引号，如 "hello"）。因为标签属性值使用英文双引号包裹，正文中的英文双引号可能导致解析器混淆。
   - 同理，避免在一段话中**成对使用**以下符号：\`**\`（加粗标记）、\`__\`（下划线标记）、\`~~\`（删除线标记）、\`==\`（强调标记）、\`!!\`（胶囊标签标记）、\`^^\`（强强调标记）、\`::\`（柔光强调标记）、\`<>\`（多图并排标记）、\`---\`（分隔线/YAML 边界）等。这些符号**成对出现**时会被解析为格式标记，单个出现通常不会引起问题。
   - 如需引用或强调包含这些符号的内容，请改用中文符号或加粗/斜体等替代方式。`;
...
7. **遵循原文原则**：尽量遵循用户提供的原文内容，如无必要，不得随意更改用户内容。仅做必要的格式调整和排版优化，不要修改原文的核心事实、数据、名称和表述。
8. **符号避让规则（防止标签误解析）**：
   - 正文避免使用**英文双引号**（即半角引号 \`"\`，如 "hello"），改用**中文双引号**（即全角引号，如 "hello"）。因为标签属性值使用英文双引号包裹，正文中的英文双引号可能导致解析器混淆。
   - 同理，避免在一段话中**成对使用**以下符号：\`**\`（加粗标记）、\`__\`（下划线标记）、\`~~\`（删除线标记）、\`==\`（强调标记）、\`!!\`（胶囊标签标记）、\`^^\`（强强调标记）、\`::\`（柔光强调标记）、\`<>\`（多图并排标记）、\`---\`（分隔线/YAML 边界）等。这些符号**成对出现**时会被解析为格式标记，单个出现通常不会引起问题。
   - 如需引用或强调包含这些符号的内容，请改用中文符号或加粗/斜体等替代方式。`;
...
export function buildCardAiGuide(aspect: string): string {
  ...
  return [
    ...
    "- **符号避让规则**：正文避免使用**英文双引号**（即半角引号 `\"`，如 \"hello\"），改用**中文双引号**（即全角引号，如 \u201Chello\u201D）。避免在一段话中**成对使用**以下符号：`**`、`__`、`~~`、`==`、`!!`、`^^`、`::` 等，这些符号成对出现时会被误解析为格式标记，单个出现通常不会引起问题。",
    "",
    "## 分页建议",
    "",
